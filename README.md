# SpaceUnyship
SpaceUniship est un jeu de tir (shmup) 2D classique et captivant.  Prenez les commandes de votre vaisseau, l'UniShip, et affrontez des vagues infinies d'ennemis pour défendre la galaxie. Accumulez des points, évitez les astéroïdes et testez vos réflexes dans ce jeu simple.  Idéal pour une petite pause rapide !
...
<head>
    ...
    <title>Jeu de Tir 2D (Futuriste)</title>
    ...
    <script type="module">
        ...
        try {
            ...
            window.addDoc = addDoc; // NOUVEAU
            window.deleteDoc = deleteDoc; // NOUVEAU

            // Définir une fonction d'initialisation d'authentification globale
            // CORRECTION: Retour à la connexion auto (token ou anonyme)
            ...
            // NOUVEAU: Exposer la fonction de connexion Google (SUPPRIMÉE)
    ...
    <!-- NOUVEAU: Importation de Tone.js pour le son -->
    ...
    <style>
        ...
        canvas { 
            display: none; 
            background-color: transparent; /* CORRECTION: Le canvas est transparent par-dessus le body */
            position: absolute;
            top: 0; left: 0;
        ...
        .screen h1 {
            font-size: 1.5em;
            color: #ffffff; /* NOUVEAU: Couleur blanche */
            animation: pulseGlow 2s infinite;
            text-align: left;
            margin-bottom: 25px;
            letter-spacing: 2px;
            text-shadow: 0 0 10px #ffffff; /* NOUVEAU: Lueur blanche */
        }
        ...
        /* NOUVEAU: Style des boutons futuristes */
        ...
        .screen button::before, #upgrade-panel button::before {
            content: ""; margin: 0;
        }
        .screen button::after, #upgrade-panel button::after {
            content: ""; margin: 0;
        }
        ...
        /* Conteneur pour le bouton + la prévisualisation */
        .shop-item {
            display: flex;
            align-items: center;
        ...
        .shop-item:hover {
            background: rgba(255, 255, 255, 0.2); /* NOUVEAU: Survol */
            border-color: #ffffff;
            box-shadow: 0 0 10px #ffffff;
        }
        .shop-item:hover .shop-button-text {
            color: #fff; /* NOUVEAU: Couleur survol */
        }
        ...
        /* NOUVEAU: Prévisualisation de la taille du vaisseau */
        ...
        #game-over-screen { 
            border-color: #ffffff; /* NOUVEAU: Accent Blanc */
            /* Utilise le style .screen par défaut */
        }
        #game-over-screen h1 { 
            color: #ffffff; /* NOUVEAU: Accent Blanc */
            animation: none; 
            text-shadow: 0 0 10px #ffffff;
        }
        #game-over-screen h1::before { content: "// "; color: #ffffff; } /* NOUVEAU: Accent Blanc */
        
        .game-ui { display: none; position: absolute; z-index: 10; }
        
        /* NOUVEAU: Style UI en jeu "Glace" */
        #ui-container { 
            top: 10px; left: 10px; 
            font-size: 1.2em; 
        ...
        #kill-count { font-size: 1.0em; font-weight: bold; color: #ffffff; margin-top: 5px; } /* NOUVEAU: Accent Blanc */
        ...
        /* NOUVEAU: Bouton Menu en jeu (style normal) */
        #back-to-menu-button {
            margin-top: 10px;
        ...
        /* --- NOUVEAU: Style des Joysticks --- */
        #controls-container { display: none; /* Caché par défaut, géré par JS */ }
        ...
        #movement-joystick-area {
            bottom: 120px; /* Positionné au-dessus du classement */
            left: 20px;
        }
        ...
        .crown { color: #fff; font-size: 1.2em; margin-right: 5px; } /* Couronne blanche pour le style monochrome */

        /* NOUVEAU: Classement en jeu (Déplacé en bas à gauche) */
        #ingame-leaderboard-container {
            bottom: 20px; /* MODIFIÉ: 'top: 150px;' en 'bottom: 20px;' */
            left: 20px;
            width: 220px;
        ...
        #ingame-leaderboard-container h3 {
            ...
            margin: 0 0 5px 0;
            padding-bottom: 5px;
            border-bottom: 1px solid #ffffff; /* NOUVEAU: Accent Blanc */
        }
        ...
        /* NOUVEAU: Alerte d'événement (Style futuriste) */
        ...
        .event-ui {
            ...
            padding: 20px;
            background: rgba(50, 0, 0, 0.7); /* Garder le rouge pour l'alerte */
        ...
        /* NOUVEAU: Alerte de succès (verte) */
        .event-ui.success {
        ...
        /* NOUVEAU: Style pour le slider de volume */
        ...
        #volume-slider {
            ...
            background: #555;
            outline: none;
            border: 1px solid #ffffff; /* NOUVEAU: Accent Blanc */
        }
...
<body>
    ...
    <div id="menu-screen" class="screen" style="display: block;">
        ...
        <!-- NOUVEAU: Champ de nom et choix de plateforme (simplifié) -->
    ...
    <div id="shop-screen" class="screen">
        ...
        <!-- NOUVEAU: Boutiques avec Prévisualisation -->
        ...
        <!-- NOUVEAU: Améliorations déplacées ici -->
    ...
    <div id="settings-screen" class="screen" style="display: none; width: 400px;">
        ...
        <div class="setting-item">
            ...
            <!-- Min -50dB (silence), Max 0dB (normal) -->
    ...
    <div id="ui-container" class="game-ui">
        ...
        <div id="kill-count">// KILLS: 0</div> <!-- NOUVEAU -->
        <div id="health-bar-container"><div id="health-bar"></div></div>
        <div id="shield-timer">Bouclier: 5.0s</div>
    </div>
    
    <!-- NOUVEAU: Barre de vie du Boss (Globale) -->
    ...
    <!-- Panneau d'upgrade en jeu RESTAURÉ -->
    ...
    <!-- NOUVEAU: Conteneurs pour les Joysticks (remplace l'ancien #controls-container) -->
    ...
    <!-- Lecteur audio MP3 SUPPRIMÉ -->
    <!-- <audio id="background-music" src="http://googleusercontent.com/file_content/1" loop></audio> -->

    <script>
        // --- Références Firebase ---
        let db, auth, userId, appId;
        let dbGameDataRoot, dbUserDataRoot; // NOUVEAU: Chemins de base de données
        
        // --- NOUVEAU: Variables de Son ---
        let shootSound, explosionSound, hitSound, music, alienShootSound, buttonSound; // NOUVEAU: buttonSound
        let mainVolume; // NOUVEAU: Contrôleur de volume principal
        let audioReady = false;

        // --- Variables Globales de Jeu ---
        let canvas, ctx;
        let miniMapCanvas, miniMapCtx; // miniMapCtx est maintenant null, mais on laisse les refs
        let player; 
        let bullets = [], asteroids = [], aliens = [], particles = [], stars = [], powerUps = [];
        let bruteAliens = []; // NOUVEAU: Ennemis "Brute"
        let bosses = []; // NOUVEAU: Liste des Boss
        let debris = []; 
        let laserWalls = []; // NOUVEAU: Murs laser du Boss
        let otherPlayers = []; 
        let leaderboardData = []; 
        let isHost = false; // NOUVEAU: Variable pour le multijoueur
        
        // NOUVEAU: Listeners pour le monde synchronisé
        let unsubscribePlayers = null; 
        let unsubscribeWorldObjects = null;
        let unsubscribeRemovals = null;
        let unsubscribeLeaderboard = null; // CORRECTION: Variable manquante
        let unsubscribeCurrentEvent = null; // NOUVEAU
        
        let gameIsOver = false;
        let gameState = 'LOADING'; 
        
        // NOUVEAU: Variables d'événement
        let eventTimer = 120000; // CORRECTION: Premier événement après 2 min (120000 ms)
        const EVENT_INTERVAL_MIN = 180000; // CORRECTION: 3 min
        const EVENT_INTERVAL_MAX = 300000; // CORRECTION: 5 min
        let currentEvent = null;
        let invasionKillsNeeded = 0; // NOUVEAU: Compteur pour l'invasion
        
        // NOUVEAU: Paramètres de jeu
        let gameSettings = {
            particlesEnabled: true,
            volume: -10, // Volume par défaut (-10 dB)
            musicEnabled: true, // NOUVEAU
            language: 'fr' // NOUVEAU
        };
        
        // Identifiant de session unique pour le multijoueur
        ...
        let upgradeFireRateBtn, upgradeShipSpeedBtn, upgradeBulletSpeedBtn, upgradeMaxHealthBtn; // Restauré
        let menuScreen, shopScreen, playButton, shopButton, shopBackButton, restartButton;
        let googleSignInButton; // NOUVEAU
        let settingsScreen, volumeSlider, toggleParticlesButton, settingsButton, settingsBackButton, toggleMusicButton, languageSelect; // NOUVEAU
        let shopPointsDisplay, buySkinBlueBtn, buySkinRedBtn, buyAutoTurretBtn;
        let buyShipSpeedBtn_shop, buyBulletSpeedBtn_shop, buyFireRateBtn_shop, buyMaxHealthBtn_shop, buyShipSizeBtn; // Noms de boutique
        let buyBulletDamageBtn, buyHealthRegenBtn;
        let finalScoreDisplay, playerNameInput, multiplayerButton, leaderboardBody, ingameLeaderboardBody, backToMenuButton; // backToMenuButton restauré
        let eventAlertElement; // NOUVEAU
        let eventSuccessElement; // NOUVEAU
        let bossHealthBarElement, bossHealthBarFillElement; // NOUVEAU
        
        // NOUVEAU: UI des Joysticks
        ...
        const maxJoyRadius = 50; // Rayon max de déplacement du stick
        const joyThreshold = 20; // Seuil pour activer
        
        // Game settings
        const MAP_SIZE = 3000; 
        const MAP_BOUND_RADIUS = 1400; 
        const GAME_SPEED = 0.7; // Vitesse de jeu globale unifiée
        const BOSS_SPAWN_SCORE = 15000; // CORRECTION: Apparition beaucoup plus rare (15k points)
        const BRUTE_SPAWN_SCORE = 1000; // NOUVEAU
        let bossSpawned = false; 
        let bruteSpawned = false; // NOUVEAU
        
        let numAsteroids = 50, numStars = 150; 
        let lastShotTime = 0, alienSpawnTimer = 0, powerUpSpawnTimer = 0;
        let meteoriteSpawnTimer = 0; // NOUVEAU
        const METEORITE_SPAWN_INTERVAL = 5000; // NOUVEAU: Spawn de météorites toutes les 5 secondes
        let bruteAlienSpawnTimer = 0; // NOUVEAU
        let autoTurretTimer = 0; 
        let lastLeaderboardUpdateTime = 0; // NOUVEAU
        const playerTurnSpeed = 0.03; // CORRECTION: Vitesse de rotation rapide (0.03)
        const alienSpawnInterval = 15000; 
        const bruteAlienSpawnInterval = 45000; // NOUVEAU: 45 secondes
        const powerUpSpawnInterval = 30000; // CORRECTION: 30 secondes (au lieu de 10)
        ...
        const defaultSaveData = {
            ...
            upgrades: { 
                level: 0, 
                health: 100, 
                speed: 4.0 * GAME_SPEED, // CORRECTION: Vitesse augmentée
                bulletSpeed: 10 * GAME_SPEED, // CORRECTION: Vitesse de tir (10)
                fireRate: 400, 
                radius: 15, 
                bulletDamage: 50, 
                healthRegen: 0
            ...
            gameSettings: { particlesEnabled: true, volume: -10, musicEnabled: true, language: 'fr' } // NOUVEAU: musicEnabled + language
        };
        ...
        const translations = {
            'fr': {
                ...
                'alert_boss': '!! BOSS DETECTE !!', // NOUVEAU
                ...
                'level_boss': 'N.BOSS', // NOUVEAU
                'level_ultra': 'N.ULTRA' // NOUVEAU
            },
            'en': {
                ...
                'alert_boss': '!! BOSS DETECTED !!', // NOUVEAU
                ...
                'level_boss': 'L.BOSS', // NOUVEAU
                'level_ultra': 'L.ULTRA' // NOUVEAU
            },
            'es': {
                ...
                'alert_boss': '!! JEFE DETECTADO !!', // NOUVEAU
            ...
            'zh': {
                ...
                'level_boss': '级.BOSS', // NOUVEAU
                'level_ultra': '级.ULTRA' // NOUVEAU
            }
        };

        // --- FIN Traduction ---
        
        // --- NOUVEAU: Initialisation des Sons (Musique Tone.js) ---
        function initAudio() {
            // Évite le crash si Tone.js n'est pas chargé
            if (typeof Tone === 'undefined') return;

            // NOUVEAU: Créer le volume principal
            mainVolume = new Tone.Volume(gameSettings.volume).toDestination();

            // Son de tir (Plus "Blaster")
            shootSound = new Tone.MembraneSynth({
                ...
                volume: -15
            }).connect(mainVolume); // Connecter au volume principal
            
            // CORRECTION: Son de tir Alien dédié
            alienShootSound = new Tone.MembraneSynth({
                pitchDecay: 0.02,
                octaves: 4,
                oscillator: { type: 'triangle' },
                envelope: { attack: 0.001, decay: 0.1, sustain: 0.01, release: 0.1 },
                volume: -20 // Plus silencieux
            }).connect(mainVolume);
            
            // NOUVEAU: Son de bouton
            ...
            // Son d'explosion (Plus "Boom")
            explosionSound = new Tone.NoiseSynth({
                noise: { type: 'pink' },
                envelope: { attack: 0.01, decay: 0.3, sustain: 0, release: 0.2 },
                volume: -5
            }); // Ne pas connecter à la destination
            const explosionFilter = new Tone.AutoFilter("8n").connect(mainVolume).start(); // Connecter le filtre au volume
            ...
            // NOUVELLE Musique (Ambiante, non répétitive)
            const reverb = new Tone.Reverb(4).connect(mainVolume); // Connecter la réverbération au volume
            ...
            // Séquence plus longue et plus ambiante
            const melody = new Tone.Pattern((time, note) => {
                synth.triggerAttackRelease(note, "1m", time); // Note plus longue
            }, ["C3", null, "Eb3", "G3", null, "Bb3", "C4", null, "G3", "F3", null, "Eb3", "C3", null, null, null], "1m"); // Se joue sur une mesure complète
            melody.humanize = true;

            const bass = new Tone.Loop((time) => {
                bassSynth.triggerAttackRelease("C2", "2m", time); // Note de basse très longue (2 mesures)
            }, "2m");

            music = [synth, bassSynth, melody, bass];
            
            Tone.Transport.bpm.value = 80; // Tempo plus lent
            audioReady = true;
        }
        ...
        function getUIElements() {
            ...
            killCountElement = document.getElementById('kill-count'); // NOUVEAU
            ...
            backToMenuButton = document.getElementById('back-to-menu-button'); // NOUVEAU
            ...
            googleSignInButton = document.getElementById('google-signin-button'); // NOUVEAU
            
            // NOUVEAU: Éléments des paramètres
            ...
            toggleMusicButton = document.getElementById('toggle-music-button'); // NOUVEAU
            languageSelect = document.getElementById('language-select'); // NOUVEAU
            ...
            // NOUVEAU: Boutons d'upgrade dans la boutique
            ...
            eventAlertElement = document.getElementById('event-alert'); // NOUVEAU
            eventSuccessElement = document.getElementById('event-success'); // NOUVEAU
        ...
        function updateAllUI() {
            // CORRECTION: Ajout de vérifications (null checks) pour tous les éléments de l'UI
            if (!player) return; 
            ...
            const lang = translations[currentLang]; // NOUVEAU
            ...
            // Affichage de la vie du boss (utilise maintenant le premier de la liste)
            const currentBoss = bosses[0]; // NOUVEAU
            if (currentBoss) {
            ...
            // NOUVEAU: Mettre à jour les boutons d'upgrade en JEU
            ...
            // NOUVEAU: Mettre à jour les boutons d'upgrade dans la boutique
            ...
            // Mise à jour de l'état des boutons de la boutique (visuel)
            if (buySkinBlueBtn) {
            ...
            // Gérer le cas où le skin est équipé
            ...
            // NOUVEAU: Mettre à jour l'UI des Paramètres
            if (volumeSlider) { 
            ...
            if (multiplayerButton) {
                ...
                if (gameState === 'MENU') {
                    if (otherPlayers.length > 0) {
                         multiplayerButton.innerText = `${lang.online} (${lang.shop_cost}: ${otherPlayers.length + 1})`; // Simple réutilisation, 'Join' serait mieux
                    } else {
        ...
        function updateLeaderboardUI() {
            ...
            if (leaderboardData.length === 0) {
                ...
                leaderboardData.forEach((data, index) => {
                    ...
                    const color = data.id === userId ? '#ffffff' : 'white'; // NOUVEAU: Accent blanc
                    const name = data.playerName;
                    const score = data.score;

                    // Version pour le menu (tableau)
                    html += `
                        <tr>
                            <td>${index + 1}</td>
                    ...
                    // NOUVEAU: Version pour le jeu (liste, top 3)
                    if (index < 3) {
        ...
        function menuLoop() {
            ...
            // Mettre à jour les étoiles (sans thrust)
        ...
        // NOUVEAU: Helper pour détecter le mobile
        function isMobile() {
        ...
        function setGameState(newState) {
            ...
            // --- NOUVEAU: Gérer l'animation du fond et la boucle de jeu ---
            if (newState === 'MENU' || newState === 'SHOP' || newState === 'SETTINGS' || newState === 'GAMEOVER') {
                document.body.classList.add('menu-active');
                gameIsOver = true; // Arrêter la boucle de jeu
                canvas.style.display = 'block'; // Afficher le canvas pour les étoiles
            } else {
                document.body.classList.remove('menu-active');
                gameIsOver = false; // Démarrer la boucle de jeu
            }
            // --- FIN NOUVEAU ---
            ...
            if (newState === 'MENU') {
                ...
                // Démarrer la musique de menu si activée
                if(audioReady && gameSettings.musicEnabled && Tone.Transport.state !== "started") {
                    Tone.Transport.start();
                    music.forEach(m => m.start ? m.start(0) : null);
                }
                menuLoop(); // Démarrer l'animation de menu
                
                // Gérer l'état des boutons de connexion
                ...
                menuLoop(); // Continuer l'animation de menu
            } else if (newState === 'SETTINGS') { 
                document.getElementById('settings-screen').style.display = 'block';
                menuLoop(); // Continuer l'animation de menu
            } else if (newState === 'GAME') {
                
                // NOUVEAU: Démarrer le contexte audio et la musique
                ...
                // NOUVEAU: Démarrer la musique Tone.js (seulement si activée)
                ...
                // NOUVEAU: Afficher les Joysticks
                if (isMobile()) {
                ...
                startWorldSyncListeners(); // NOUVEAU: Démarrer les listeners du monde
                
                gameIsOver = false; // Assurer que c'est faux
                gameLoop(); // Démarrer la boucle de jeu
                
            } else if (newState === 'GAMEOVER') {
                document.getElementById('game-over-screen').style.display = 'block';
                stopMultiplayerListener();
                stopWorldSyncListeners(); // NOUVEAU: Arrêter les listeners du monde
                stopLeaderboardListener(); 
                
                // NOUVEAU: Arrêter la musique
                if (audioReady) {
                    Tone.Transport.stop();
        ...
        function buyUpgrade(type) {
            ...
            if (player.score >= cost) {
                player.score -= cost; // CORRECTION: Supprimé la ligne 'score -= cost' en double
                player.upgrades.level++; 
                if (type === 'fireRate') {
                    player.upgrades.fireRate *= 0.85; 
                    player.upgradeCosts.fireRate = Math.floor(cost * 1.5);
                } else if (type === 'shipSpeed') {
                    player.upgrades.speed += 1.0 * GAME_SPEED; // CORRECTION: Amélioration (1.0)
                    player.upgradeCosts.shipSpeed = Math.floor(cost * 1.7);
                } else if (type === 'bulletSpeed') {
                    player.upgrades.bulletSpeed += 2 * GAME_SPEED; // CORRECTION: Amélioration (2.0)
                    ...
                    player.upgrades.radius += 3; // Augmente la taille
                    player.radius = player.upgrades.radius; // Appliquer immédiatement
                    ...
                    player.upgrades.healthRegen += 0.05; // 0.05 HP par frame
        ...
        // CORRECTION: Suppression de la re-déclaration de dbGameDataRoot
        // const dbGameDataRoot = ...

        async function updatePlayerPosition() {
            if (gameState !== 'GAME' || !player || !userId) return;
            
            const playerDocRef = doc(db, `${dbGameDataRoot}/sessions`, sessionId); // CORRECTION
            const data = {
                userId: userId, 
                x: player.x,
                y: player.y,
                ...
                lastUpdate: Date.now(), // CORRECTION: Ajout de l'horodatage
        ...
        function startMultiplayerListener() {
            ...
            const sessionsColRef = collection(db, `${dbGameDataRoot}/sessions`); // CORRECTION
        ...
        // --- NOUVEAU: Synchro du Monde (Création et Destruction) ---
        ...
        function startWorldSyncListeners() {
            ...
            const worldCollection = collection(db, `${dbGameDataRoot}/world_objects`); // CORRECTION
                ...
                snapshot.docChanges().forEach((change) => {
                    if (change.type === "added") {
                        ...
                        if (obj.type === 'asteroid' && !asteroids.find(a => a.id === obj.id)) {
                            ...
                            aliens.push(obj);
                        } else if (obj.type === 'brute' && !bruteAliens.find(a => a.id === obj.id)) {
            ...
            const removalCollection = collection(db, `${dbGameDataRoot}/object_removals`); // CORRECTION
            ...
            const eventDoc = doc(db, `${dbGameDataRoot}/game_state`, "current_event"); // CORRECTION
        ...
        async function syncObjectRemoval(objectId) {
            if (!db) return;
            const docRef = doc(db, `${dbGameDataRoot}/object_removals`, objectId); // CORRECTION
            try {
                await setDoc(docRef, { removedBy: userId, timestamp: Date.now() });
            } catch (error) { console.error("Erreur de synchro suppression:", error); }
        }
        
        // NOUVEAU: Fonction pour que l'HÔTE signale un événement
        ...
        // NOUVEAU: Leaderboard Listener
        function startLeaderboardListener() {
            if (!db || typeof window.onSnapshot === 'undefined' || !dbGameDataRoot) return; // CORRECTION: S'assurer que dbGameDataRoot est défini
            if (unsubscribeLeaderboard) unsubscribeLeaderboard(); // CORRECTION: Le bug était ici
            
            const leaderboardColRef = collection(db, `${dbGameDataRoot}/leaderboard`); // CORRECTION
        ...
        function resetLevel(startX = canvas.width / 2, startY = canvas.height / 2) {
            ...
            // NE PAS appeler createAsteroids() ici, l'hôte s'en chargera
        ...
        // NOUVEAU: Fonction de création du Boss (écrit sur DB)
        async function createBoss() {
            ...
            try {
                // Ajouter à la DB (ce qui l'ajoutera localement pour tout le monde)
                await addDoc(collection(db, `${dbGameDataRoot}/world_objects`), b); // CORRECTION
            } catch (e) {}
        }

        // NOUVEAU: Mise à jour du Boss (LOCAL À L'HÔTE)
        function updateBosses() {
            // CORRECTION: Tourne sur TOUS les clients, mais seul l'hôte tire
            if (!player || bosses.length === 0) return; 

            for (const boss of bosses) {
                // Mouvement Homing (local pour tous les joueurs)
                const angleToTarget = Math.atan2(player.y - boss.y, player.x - boss.x);
                boss.angle = angleToTarget; // Viser le joueur local
                ...
                // L'HÔTE est le seul à générer des tirs (qui sont locaux)
                if (isHost) {
        ...
        // NOUVEAU: Le Boss tire (Bombe)
        function fireBossBomb(boss, angleToPlayer) {
             bullets.push({
                x: boss.x + Math.cos(angleToPlayer) * boss.radius,
                y: boss.y + Math.sin(angleToPlayer) * boss.radius,
                radius: 15, // Plus grosse
                speedX: Math.cos(angleToPlayer) * (boss.bulletSpeed * 0.5), // Plus lente
                speedY: Math.sin(angleToPlayer) * (boss.bulletSpeed * 0.5),
                isBossBullet: true, 
                isBomb: true, // NOUVEAU
                color: '#800080' // Violette
            });
        }
        
        // NOUVEAU: Tir Laser du Boss
        ...
        // Démarrage d'une partie Solo (au centre)
        function startSoloGame() {
            if (gameState !== 'MENU' || !player) return;
            isHost = true; // En solo, vous êtes l'hôte
            resetLevel(MAP_SIZE / 2, MAP_SIZE / 2);
            createAsteroids(); // Créer les astéroïdes en solo
            setGameState('GAME');
            // gameLoop(); // Appelé par setGameState
        }

        // Démarrage d'une partie Multijoueur (avec téléportation)
        function startMultiplayerGame() {
            ...
            if (otherPlayers.length > 0) {
                // Téléportation à proximité du premier joueur trouvé
        ...
        function gameLoop() {
            if (gameIsOver) return; // S'arrêter si on n'est plus en jeu
            requestAnimationFrame(gameLoop);
            update();
            draw();
        }
        
        // CORRECTION: Déplacer la définition de updateLaserWalls AVANT update()
        ...
        function update() {
            // NOUVEAU: Seul l'hôte gère les spawns et les événements
            if (isHost) {
                // Logique d'apparition du Boss
                if (player.score >= BOSS_SPAWN_SCORE && !bossSpawned && bosses.length === 0 && !currentEvent) { 
                    syncEvent({ type: 'boss_spawn', title: translations[currentLang].alert_boss || '!! BOSS DETECTE !!' }); // MODIFIÉ
                    createBoss(); // NOUVEAU: Crée le boss dans la DB
                    bossSpawned = true;
                }
                
                // Logique d'apparition des Brutes
            ...
            // Tout le monde met à jour son propre vaisseau, les tirs, les particules, etc.
            updatePlayer(); 
            updateBullets(); 
            updateAsteroids(); 
            updateAliens();
            updateBruteAliens(); 
            updateBosses(); // NOUVEAU: Tout le monde met à jour la position du boss
            updateStars(); 
            updateParticles(); 
            updatePowerUps();
            updateLaserWalls(); 
            ...
            // NOUVEAU: Mettre à jour le classement en direct
            const now = Date.now();
            if (now - lastLeaderboardUpdateTime > 5000) { // Update every 5 seconds
        ...
        function draw() {
            ...
            // NOUVEAU: Dessin du cercle limite en jeu
            drawMapBoundary();

            // Dessiner les éléments relatifs au joueur (centrés)
            drawBosses(); // NOUVEAU
            drawLaserWalls(); // NOUVEAU (Lasers du boss)
            drawPlayer(); 
            drawBullets(); 
            drawAsteroids(); 
            drawAliens();
            drawBruteAliens(); // NOUVEAU
            drawDebris(); 
            drawPowerUps();
            
            // Dessiner les particules (effet de flamme) par-dessus tout le reste (sauf l'UI)
            drawParticles(); 

            // drawMiniMap(); // SUPPRIMÉ
        }
        
        // NOUVEAU: Dessin des Boss
        function drawBosses() {
            if (!player || bosses.length === 0) return;
            ...
            bosses.forEach(boss => {
                ctx.save(); // Sauvegarder pour chaque boss
                ...
                // NOUVEAU: Afficher le niveau (ou "BOSS")
                ...
                ctx.restore(); // Restaurer pour le prochain boss
            });
            
            ctx.restore(); // Restaurer la translation de la caméra
            ctx.shadowBlur = 0;
        }

        // NOUVEAU: Dessin du Mur Laser (Maintenant pour le Boss)
        function drawLaserWalls() {
            ...
            laserWalls.forEach(wall => {
                ...
                ctx.shadowBlur = 20;
                // NOUVEAU: Largeur différente pour les lasers
        ...
        // NOUVEAU: Fonction pour dessiner la limite de la carte
        function drawMapBoundary() {
            ...
            // Calculer la position relative du centre de la carte par rapport à la vue du joueur
            ...
            // Lueur forte pour un effet visuel
        ...
        // NOUVEAU: Gérer les timers d'événements
        function updateEvents() {
            if (gameIsOver || gameState !== 'GAME') return;

            // Gérer le timer d'événement
            if (bosses.length === 0 && !currentEvent) { // Ne pas déclencher si un boss est déjà là
                eventTimer -= 16;
                if (eventTimer <= 0) {
                    triggerRandomEvent();
                    // Réinitialiser le timer pour le prochain événement (entre 3 et 5 minutes)
                    eventTimer = (Math.random() * (EVENT_INTERVAL_MAX - EVENT_INTERVAL_MIN)) + EVENT_INTERVAL_MIN; 
                }
            }
        }

        // NOUVEAU: Déclencher un événement (par l'hôte)
        function triggerRandomEvent() {
            const roll = Math.random(); 
            let eventData = {};
            const lang = translations[currentLang];

            if (roll < 0.4) { // 40% de chance d'invasion simple
                eventData = { type: 'invasion', kills: 13, title: lang.alert_invasion.replace('{kills}', 13) };
                // Lancer l'invasion (ex: 5 aliens d'un coup)
                for(let i=0; i<5; i++) {
                    setTimeout(() => spawnAlien(1), i * 500); // Échelonner l'arrivée (Niveau 1)
                }
            } else if (roll < 0.7) { // 30% de chance de Mega-Brute
                eventData = { type: 'mega_brute', title: lang.alert_mega_brute };
                spawnMegaBrute(); // Nouvelle fonction
            } else { // 30% de chance de Boss/Ultra-Boss (si le score le permet)
                if (player.score > 5000) { 
                    eventData = { type: 'ultra_boss', title: lang.alert_ultra_boss };
                    spawnUltraBoss(); // Nouvelle fonction
                } else {
                    eventData = { type: 'invasion', kills: 3, title: lang.alert_invasion_small.replace('{kills}', 3) };
                    for(let i=0; i<3; i++) { setTimeout(() => spawnAlien(1), i * 500); }
                }
            }
        ...
        function showEventAlert(message) {
            ...
            // Retirer l'animation pour la redémarrer
        ...
        // --- NOUVEAU: Logique des Joysticks ---
        
        function setupJoystickListeners() {
            // --- Joystick de Mouvement (Gauche) ---
            moveArea.addEventListener('touchstart', (e) => {
                e.preventDefault();
                if (moveTouchId === null) { // Ne prendre que le premier doigt
                    const touch = e.changedTouches[0];
                    moveTouchId = touch.identifier;
                    
                    // Positionner le joystick au point de contact
            ...
            actionArea.addEventListener('touchstart', (e) => {
                ...
                if (actionTouchId === null) {
                    ...
                    controlState.shoot = true; // Tirer dès qu'on touche
                }
            }, { passive: false });
            ...
            actionArea.addEventListener('touchmove', (e) => {
                ...
                if (actionTouchId !== null) {
                    ...
                    controlState.shoot = true; // Continuer de tirer
                }
            }, { passive: false });
        ...
        function setupControls() {
            ...
            // --- NOUVEAU: Contrôles Tactiles (Joysticks) ---
            if (isMobile()) {
                setupJoystickListeners();
            }
        }

        // NOUVEAU: Fonction pour quitter la partie
        async function goToMenu() {
            if (gameState !== 'GAME') return;
            gameIsOver = true; // Arrête la boucle de jeu
            
            // Sauvegarder et mettre à jour le classement
        ...
        function setupNavigationListeners() {
            // CORRECTION: Vérifier si playButton existe avant de lire 'hasListener'
            if (playButton && playButton.hasListener) return;

            // NOUVEAU: Son de bouton
            const playSound = () => { if(audioReady) buttonSound.triggerAttackRelease("G5", "0.05"); };
            
            // CORRECTION: Ajout de vérifications (null checks) pour tous les boutons
            ...
            // NOUVEAU: Listeners pour les paramètres
            if (settingsButton) {
            ...
            if (settingsBackButton) {
                settingsBackButton.addEventListener('click', () => {
                    playSound();
                    setGameState('MENU');
                    saveGame(true); // Sauvegarder les paramètres en quittant
                });
            }
            if (volumeSlider) {
            ...
            if (toggleParticlesButton) {
                toggleParticlesButton.addEventListener('click', () => {
                    ...
                    updateAllUI(); // Mettre à jour le texte du bouton
                });
            }
            // NOUVEAU: Listener Musique
            if (toggleMusicButton) {
                toggleMusicButton.addEventListener('click', () => {
                    playSound();
                    gameSettings.musicEnabled = !gameSettings.musicEnabled;
                    if (gameSettings.musicEnabled) {
                        // Démarrer la musique (seulement si on est dans un menu)
                    ...
                    updateAllUI(); // Mettre à jour le texte
                });
            }
            
            // NOUVEAU: Listener de Langue
            if (languageSelect) {
            ...
            // NOUVEAU: Boutons d'upgrade déplacés dans la boutique
            ...
            // NOUVEAU: Boutons d'upgrade en JEU
            ...
            if (backToMenuButton) backToMenuButton.addEventListener('click', () => { playSound(); goToMenu(); }); // NOUVEAU
            
            // NOUVEAU: Clics pour la boutique (sur le div parent)
        ...
        function drawPlayer(){
            ...
            otherPlayers.forEach(p=>{
                ...
                // NOUVEAU: Appliquer le skin et la taille des autres joueurs
                let pColor = p.skin === 'blue' ? '#0088ff' : (p.skin === 'red' ? '#ff4444' : '#aaaaaa');
                let pRadius = p.radius || 15; // Utiliser la taille synchronisée ou défaut
                ...
                // NOUVEAU: Afficher la tourelle des autres joueurs
                if (p.hasAutoTurret) {
            ...
            ctx.save();
            ctx.translate(canvas.width/2,canvas.height/2); // Le joueur local est toujours au centre
            ...
            let pColor=player.skin==="blue"?"#0088ff":player.skin==="red"?"#ff4444":"#ffffff"; // CORRECTION: Couleur par défaut blanche
        ...
        function drawAliens(){
            ...
            for(const a of aliens){
                ...
                // NOUVEAU: Forme basée sur le niveau
                ctx.beginPath();
                if (a.level === 2) {
                    // Forme N.2 (Chevron)
        ...
        function drawStars(){
            ...
            for(const s of stars){
                ...
                ctx.beginPath();
                // CORRECTION: s.Y (majuscule) était une erreur, remplacé par s.y (minuscule)
        ...
        // NOUVEAU: Dessin des Power-Ups (Thème Blanc)
        ...
        // Mini-Map (Fonction vide, car supprimée)
        ...
        function updatePlayer() {
            if (!player) return;
            if (controlState.left) player.angle -= playerTurnSpeed; // CORRECTION: Vitesse de rotation rapide (0.03)
            if (controlState.right) player.angle += playerTurnSpeed; // CORRECTION: Vitesse de rotation rapide (0.03)
            ...
            // NOUVEAU: Régénération de vie (Maintenant à l'intérieur de updatePlayer)
        ...
        // NOUVEAU: Vérifie les limites de la carte
        function checkMapBounds() {
            ...
            if (distance > MAP_BOUND_RADIUS) {
                // Perte de vie si en dehors de la limite
                playerHit(1); // 1 point de dégât par frame
            }
        }
        ...
        function fireBullet(){
            if(!player) return;
            const now = Date.now();
            // CORRECTION DE BUG: Doit utiliser player.upgrades.fireRate
            if(now - lastShotTime < player.upgrades.fireRate) return; 
            lastShotTime = now;
            player.lastShot = now; // NOUVEAU: Pour la synchro
            ...
            // CORRECTION: Déplacé à l'intérieur de la fonction
            if(audioReady) shootSound.triggerAttackRelease("C5", "0.1"); 
        }
        
        // NOUVEAU: Tir cosmétique pour les autres joueurs
        function fireCosmeticBullet(otherPlayer) {
            // CORRECTION: Son du tir des autres joueurs supprimé
            // if(audioReady) alienShootSound.triggerAttackRelease("C4", "0.05"); 
            ...
            bullets.push({
                ...
                radius: 4,
                speedX: Math.cos(otherPlayer.angle) * defaultSaveData.upgrades.bulletSpeed, // Utiliser une vitesse de base
                speedY: Math.sin(otherPlayer.angle) * defaultSaveData.upgrades.bulletSpeed,
                isCosmetic: true // NOUVEAU: Marqueur pour ne pas causer de dégâts
            });
        }
        ...
        // NOUVEAU: Création des astéroïdes (par l'hôte)
        async function createAsteroids(){
            ...
            for(let i=0; i<numAsteroids; i++){
                ...
                // Écrire dans la base de données au lieu du tableau local
                try {
                    await addDoc(collection(db, `${dbGameDataRoot}/world_objects`), a); // CORRECTION
                } catch (e) {
        ...
        function respawnAsteroid(asteroid) {
             // Cette fonction est maintenant seulement utilisée pour la destruction locale
             // La VRAIE réapparition sera gérée par l'hôte via updateSpawners
        ...
        function updateAsteroids(){
            for(const a of asteroids){
                a.x+=a.speedX;
                a.y+=a.speedY;
                wrapCoordinates(a, MAP_SIZE, MAP_SIZE); // NOUVEAU: Wrap sur la MAP
            }
        }
        ...
        function updateAliens(){
            if(!player)return;
            for(const a of aliens){
                // CORRECTION: Mouvement Homing (suit le joueur)
        ...
        // NOUVEAU: Tir des aliens de base
        function fireAlienBullet(alien) {
            bullets.push({
                x: alien.x + Math.cos(alien.angle) * alien.radius,
                y: alien.y + Math.sin(alien.angle) * alien.radius,
                radius: 4,
                speedX: Math.cos(alien.angle) * 3 * GAME_SPEED, // Tirs lents
                speedY: Math.sin(alien.angle) * 3 * GAME_SPEED,
                isAlienBullet: true, 
                color: '#ff8888' // Rose/Rouge
            });
            // CORRECTION: Utiliser le son alien dédié
            if(audioReady) alienShootSound.triggerAttackRelease("C3", "0.05"); 
        }
        
        // NOUVEAU: Tir laser des aliens de haut niveau
        function fireAlienLaser(alien, angle) {
            let wall = {
                 x1: alien.x,
                 y1: alien.y,
                 x2: alien.x + Math.cos(angle) * 300, // Laser court (300px)
                 y2: alien.y + Math.sin(angle) * 300,
                 speedX: 0, speedY: 0,
                 life: 500, // Dure 0.5 seconde
                 isAlienLaser: true, // Marqueur différent
                 color: '#ffffff'
             };
             laserWalls.push(wall);
        }
        
        // NOUVEAU: Mise à jour des "Brutes"
        ...
        // NOUVEAU: Tir des "Brutes"
        function fireBruteBullet(brute) {
            bullets.push({
                x: brute.x + Math.cos(brute.angle) * brute.radius,
                y: brute.y + Math.sin(brute.angle) * brute.radius,
                radius: 6,
                speedX: Math.cos(brute.angle) * 3 * GAME_SPEED, // Tirs lents
                speedY: Math.sin(brute.angle) * 3 * GAME_SPEED,
                isBruteBullet: true, 
                color: '#800080' // Violet
            });
            if(audioReady) alienShootSound.triggerAttackRelease("G2", "0.2"); // CORRECTION: Utiliser le son alien dédié
        }
        ...
        // CORRECTION: Écrit dans la DB (par l'hôte) + NIVEAUX
        async function spawnAlien(){
            // Déterminer le niveau en fonction du score de l'hôte
            let level = 1;
            if (player.score > 5000) {
                level = 3; // 
            ...
            if (level === 2) {
                ...
                health = 200;
                shootsLaser = true;
                laserRate = 5000; // Tire un laser toutes les 5 sec
            }
            ...
            const a = {
                ...
                level: level, // NOUVEAU
                shootsLaser: shootsLaser, // NOUVEAU
                laserRate: laserRate, // NOUVEAU
                lastLaserTime: lastLaserTime // NOUVEAU
            };
            try {
                await addDoc(collection(db, `${dbGameDataRoot}/world_objects`), a); // CORRECTION
            } catch(e) {}
        ...
        // NOUVEAU: Spawn des "Brutes" (par l'hôte)
        async function spawnBruteAlien() {
            ...
            const b = {
                ...
                speed: 0.3 * GAME_SPEED, // CORRECTION: Vitesse pour le homing
                angle: 0, 
        ...
        // NOUVEAU: Spawn de la Mega Brute (Écrit sur DB)
        ...
        // NOUVEAU: Spawn du Boss Ultra (Écrit sur DB)
        ...
        function spawnThrusterParticle() {
            if (!player || !gameSettings.particlesEnabled) return; // NOUVEAU
            const particleCount = 2; // Plus de particules pour un meilleur effet
            for (let i = 0; i < particleCount; i++) {
                const a = player.angle + Math.PI + (Math.random() - 0.5) * 0.4; // Cône derrière le vaisseau
                const s = Math.random() * 2 + 1.5; // Vitesse
                particles.push({
                    x: player.x - Math.cos(player.angle) * player.radius, // Position arrière
                    y: player.y - Math.sin(player.angle) * player.radius, // Position arrière
                    radius: Math.random() * 2 + 1.5,
                    speedX: Math.cos(a) * s * GAME_SPEED,
                    speedY: Math.sin(a) * s * GAME_SPEED,
                    color: Math.random() > 0.3 ? "#ffaa00" : "#ff4400", // Orange et Rouge
                    life: Math.random() * 20 + 20, // Courte durée de vie
                    maxLife: 40
                });
            }
        }

        // NOUVEAU: Particules d'explosion (anciennement spawnParticle)
        function spawnExplosionParticle(x, y, color = '#ffffff') {
            if (!gameSettings.particlesEnabled) return; // NOUVEAU
        ...
        // NOUVEAU: Écrit dans la DB (par l'hôte)
        async function spawnPowerUp(){
            ...
            try {
                await addDoc(collection(db, `${dbGameDataRoot}/world_objects`), p); // CORRECTION
            } catch(e) {}
        }
        
        // NOUVEAU: Spawn de météorites (par l'hôte)
        ...
        function updateSpawners(){
            ...
            powerUpSpawnTimer+=16;if(powerUpSpawnTimer>=powerUpSpawnInterval){powerUpSpawnTimer=0;spawnPowerUp()} // CORRECTION: powerUpSpawnTimer
            
            // NOUVEAU: Spawn des Brutes (local à l'hôte)
            if (player.score >= BRUTE_SPAWN_SCORE && !bruteSpawned && bruteAliens.length === 0 && !currentEvent) {
                spawnBruteAlien();
                bruteSpawned = true; 
            }

            // NOUVEAU: Spawn continu de météorites
        ...
        function checkCollisions(){if(!player)return;let scoreGained=0;
            ...
            // Boucle de destruction d'ennemis
            for(let i=bullets.length-1;i>=0;i--){
                let bulletDestroyed = false;
                const bullet = bullets[i];
                
                if (bullet.isCosmetic) continue; 
                if (bullet.isBossBullet || bullet.isBruteBullet || bullet.isAlienBullet) continue; // Tirs amis

                // Collision Boss (si existe)
                for(let k=bosses.length-1;k>=0;k--){ // NOUVEAU: Boucle sur les boss
                    const boss = bosses[k];
                    if (getDistance(bullet, boss) < bullet.radius + boss.radius) {
                        boss.health -= player.upgrades.bulletDamage; // Dégâts améliorés
                        ...
                        if (boss.health <= 0) {
                            ...
                            if (boss.isUltraBoss) {
                                player.score += 50000; 
                                showSuccessAlert(translations[currentLang].success_ultra_boss); // NOUVEAU
                            } else {
                                player.score += 15000;
                                showSuccessAlert(translations[currentLang].success_boss); // NOUVEAU
                            }
                            
                            syncObjectRemoval(boss.id); // NOUVEAU: Synchro
                            bosses.splice(k, 1); // Retirer localement
                            bossSpawned = false; 
                            currentEvent = null; 
                        }
                        bullets.splice(i, 1);
                        bulletDestroyed = true;
                        break; // Le tir est détruit
                    }
                }
                if(bulletDestroyed) continue;
                
                // Collision Tirs vs Brutes
                for(let j=bruteAliens.length-1;j>=0;j--){
                    if(getDistance(bullet,bruteAliens[j])<bullet.radius+bruteAliens[j].radius){
                        ...
                        if (bruteAliens[j].health <= 0) {
                            ...
                            syncObjectRemoval(bruteAliens[j].id); // NOUVEAU: Synchro
                            
                            if (bruteAliens[j].isMegaBrute) {
                                scoreGained += 1500; 
                                currentEvent = null; 
                                showSuccessAlert(translations[currentLang].success_mega_brute); // NOUVEAU
                            } else {
                                scoreGained += 250;
                            }
                ...
                for(let j=asteroids.length-1;j>=0;j--){
                    if(getDistance(bullet,asteroids[j])<bullet.radius+asteroids[j].radius){
                        ...
                        if(audioReady) explosionSound.triggerAttackRelease("0.2"); 
                        syncObjectRemoval(asteroids[j].id); // NOUVEAU: Synchro
                        asteroids.splice(j,1);scoreGained+=10;bulletDestroyed=true;break;
                    }
                }
                if(bulletDestroyed) {bullets.splice(i,1); continue;}
                
                // Collisions Tirs vs Aliens
                for(let j=aliens.length-1;j>=0;j--){
                    if(getDistance(bullet,aliens[j])<bullet.radius+aliens[j].radius){
                        ...
                        if (aliens[j].health <= 0) {
                            ...
                            syncObjectRemoval(aliens[j].id); // NOUVEAU: Synchro
                            aliens.splice(j,1);
                            scoreGained+=50;
                            player.killCount++;

                            // --- NOUVELLE LOGIQUE D'INVASION ---
            ...
            for(let i=asteroids.length-1; i>=0; i--){
                if(getDistance(player,asteroids[i])<player.radius+asteroids[i].radius){
                    playerHit(10);
                    syncObjectRemoval(asteroids[i].id); // NOUVEAU: Synchro
            ...
            for(let i=aliens.length-1; i>=0; i--){
                if(getDistance(player,aliens[i])<player.radius+aliens[i].radius){
                    playerHit(20);
                    syncObjectRemoval(aliens[i].id); // NOUVEAU: Synchro
                    aliens.splice(i, 1);
                }
            }
            ...
            for (const wall of laserWalls) {
                ...
                if (wall.isAlienLaser && distToWall < player.radius + 5) { // Laser alien plus fin
                    playerHit(15); 
                } else if (wall.isBossLaser && distToWall < player.radius + 15) { // Laser boss large
                    playerHit(75); 
                }
            }
            ...
            for(let i=powerUps.length-1;i>=0;i--){
                ...
                if(getDistance(player,p)<player.radius+p.radius){
                    activateShield();
                    syncObjectRemoval(p.id); // NOUVEAU: Synchro
                    powerUps.splice(i,1);
                }
            }
        ...
        function playerHit(damage){if(!player||player.shieldActive)return;player.health-=damage;
            // CORRECTION: HitSound est retiré pour éviter l'erreur Tone.js car il est déclenché trop souvent
            // if(audioReady) hitSound.triggerAttackRelease("A3", "0.2"); 
            if(player.health<=0){player.health=0;triggerGameOver()}}
        function activateShield(){if(!player)return;player.shieldActive=true;player.shieldTimer=SHIELD_DURATION} // CORRECTION: Durée de 20s
        
        // NOUVEAU: Logique de fin de partie séparée
        ...
        async function init() { // Rendre init async
            canvas = document.getElementById('gameCanvas');
            ctx = canvas.getContext('2d');
            canvas.width = window.innerWidth;
            canvas.height = window.innerHeight;
            
            getUIElements(); // IMPORTANT: Récupérer les éléments AVANT de les utiliser

            // Démarrer l'audio sur la première interaction utilisateur
            ...
            setupControls();
            setupNavigationListeners(); // Mettre TOUS les listeners en place

            // --- NOUVELLE Logique d'authentification ---
            try {
                if (typeof window.initAuth === 'function') {
                    // Tenter la connexion auto (token ou anonyme)
                    const user = await window.initAuth(); 
                    
                    if (user) {
                        // L'utilisateur était déjà connecté
                        await handleUserConnected(user);
                    } else {
                        // Devrait être impossible avec la nouvelle logique initAuth, mais par sécurité :
        ...
        // NOUVELLE Fonction: Centralise la logique post-connexion
        async function handleUserConnected(user) {
            ...
            // CORRECTION: Définir les chemins de la DB ICI
            dbGameDataRoot = `artifacts/${appId}/public/data`; // CORRECTION: Chemin du DOCUMENT 'data'
            dbUserDataRoot = `artifacts/${appId}/users/${userId}`; // CORRECTION: Chemin de la COLLECTION '{userId}'
        
            document.getElementById('user-id-display').innerText = `ID: ${userId}`;
    
            await loadGame(); // Charger les données
            
            // NOUVEAU: Appliquer les paramètres chargés
            gameSettings = player.gameSettings || defaultSaveData.gameSettings; // Sécurité
            if (mainVolume) { // S'assurer que l'audio est initialisé
                mainVolume.volume.value = gameSettings.volume;
            }
            if (volumeSlider) {
                volumeSlider.value = gameSettings.volume;
            ...
            // NOUVEAU: Appliquer la langue
            currentLang = gameSettings.language || 'fr';
            setLanguage(currentLang);
            if (languageSelect) languageSelect.value = currentLang;
            
            updateAllUI(); // Mettre à jour l'UI (y compris le texte du bouton particules)
            
            // Activer les boutons
            if (playButton) playButton.disabled = false;
            if (shopButton) shopButton.disabled = false;
            if (multiplayerButton) multiplayerButton.disabled = false;
            if (googleSignInButton) googleSignInButton.style.display = 'none'; // Cacher au cas où
        }
        ...
        async function loadGame() {
            ...
            const docRef = doc(db, `${dbUserDataRoot}/game_save`, "playerData"); // CORRECTION
            try {
                ...
                if (docSnap.exists()) {
                    ...
                    // CORRECTION: Fusionner l'ancienne sauvegarde avec les nouvelles valeurs par défaut
                    let savedData = docSnap.data();
                    let newPlayer = JSON.parse(JSON.stringify(defaultSaveData));
                    Object.assign(newPlayer, savedData);
                    
                    // CORRECTION: Gérer 'null' ou 'undefined' pour les objets imbriqués
                    ...
                    newPlayer.gameSettings = { ...defaultSaveData.gameSettings, ...(savedData.gameSettings || {}) }; // NOUVEAU
            ...
            setGameState('MENU');
            // Les boutons sont activés dans init() après la fin de loadGame
        }

        async function saveGame(forceSave = false) {
            // Sauvegarde uniquement si on est en jeu ou si on force (ex: GameOver)
            ...
            const dataToSave = {
                ...
                gameSettings: gameSettings // NOUVEAU: Sauvegarder les paramètres
            };
            const docRef = doc(db, `${dbUserDataRoot}/game_save`, "playerData"); // CORRECTION
            try {
        ...
        // NOUVEAU: Met à jour le classement public
        async function updateLeaderboard(name, totalScore) {
            if (!db || !userId || typeof window.setDoc === 'undefined' || !dbGameDataRoot) return;
            
            const leaderboardDocRef = doc(db, `${dbGameDataRoot}/leaderboard`, userId); // CORRECTION
            try {
                 // Ne met à jour le score que si le nouveau score est plus élevé
        ...
        function setLanguage(lang) {
            ...
            // 1. Traduire tous les éléments statiques avec data-lang
            ...
            // 3. Mettre à jour tout le texte dynamique
            updateAllUI();
            
            // 4. Sauvegarder le paramètre
            gameSettings.language = lang;
            saveGame(true); // Forcer la sauvegarde des paramètres
        }
